...
function draw() {
  ...
  if(gameState === play) {
    ...
    if(asteroidGroup.isTouching(endline)|| asteroidGroup.isTouching(spaceShip)) {
      asteroidGroup.destroyEach();
      gameState = end;
    }
  ...
  if(gameState === instruction) {
    ...
    text(" (i.)Some asteroids are coming towords Earth.",canvas.width/2-300, canvas.height/2 - 210);
    text("  (ii.)You are a space fighter.",canvas.width/2-300,canvas.height/2-170);
    text(" (iii.) Help the people and Earth !",canvas.width/2-300,canvas.height/2-130);
    text("  (iv.)press 'space' to shoot.",canvas.width/2-300,canvas.height/2-90);
    text("  (v.)use right and left arrows to move.",canvas.width/2-300,canvas.height/2-50);
    text("(vi.)Don't let the astroids go off the screen.",canvas.width/2-300,canvas.height/2-0);
    text("  press 's' to start game.",canvas.width/2-260,canvas.height/2+40);
